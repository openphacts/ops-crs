...
namespace RSC.Compounds.DataExport
{
	...
	/// time the job is attempted, even if it fails for a different reason.
	/// Once this value reaches 255, it will no longer increment.</param>
	public delegate void FileUploaderUploadErrorEventHandler(IFileUploader sender, string localFile, string remoteFile, string error, byte attemptCount);

	/// <summary>
	/// Implementation of the <see cref="IFileUploader"/> for FTP.
	/// </summary>
	public class FtpFileUploader : IFileUploader, IFileDownloader
	{
		/// <summary>
		...
		{
			Trace.TraceInformation("Creating FtpWebRequest for path: {0}", fullRemotePath);
		...
		bool EnqueueJob(IFtpFileUploaderJob job)
		{
			lock (_jobsSync)
			{
				...
				{
					Trace.TraceError("Enqueueing job `{0}` failed: job already in queue.", job);
					return false;
				}

				// Add the job
				_jobsQueue.Enqueue(job);
				Trace.TraceInformation("Enqueueing job `{0}` successful.", job);
			}
		...
		{
			Trace.TraceInformation("CreateDirectoryIfNotExists(filePath: {0})", filePath);
				...
				try
				{
					switch (res.StatusCode)
					{
						...
						default:
							...
							Trace.TraceError(errmsg, dir, ex);
		...
		public void FtpCreateFile(string localFile, string remoteFile)
		{
			Trace.TraceInformation("FtpCreateFile(localFile: {0}, remoteFile: {1})", localFile, remoteFile);

			//Default the offset to the beginning of the file.
			long contentOffset = 0;

			//Get the full remote path rather than th relative remote path.
			remoteFile = ResolveRemotePath(remoteFile);

			//Set the temporary filename.
			string remoteTempFile = remoteFile + ".ftptmp";

			//Retrieve the file size - will be 0 if the file doesn't exist.
			long fileSize = FtpFileSize(remoteFile);

			//Check if the remote file already exists
			bool fileExists = fileSize > 0;

			if (!fileExists)
			{
				//Check to see if a temp file already exists that we can resume uploading.
				contentOffset = FtpFileSize(remoteTempFile);
			}
			else
			{
				...
				if (SkipIfExists)
				{
					Trace.TraceInformation("Skipping creating remote file `{0}` - file already exists and SkipIfExists is set.", remoteFile);
					return;
				}
				else
				{
					// Delete the existing file as we will overrite instead.
					Trace.TraceInformation("Deleting remote file `{0}` - file already exists and SkipIfExists is not set.", remoteFile);
					FtpDeleteFile(remoteFile);
				}
			}

			//Ensure that the upload directory exists.
			CreateDirectoryIfNotExists(remoteFile);

			//Get the name of just the file.
			string fileName = remoteFile.Substring(remoteFile.LastIndexOf('/') + 1);

			//Set the size of chunks to upload - 8MB.
			long BUFFER_SIZE = (1024 * 1024) * 8;
			...
			using (Stream input = File.OpenRead(fileName))
			{
				...
				//{
				//Select the smallest of the BUFFER_SIZE and the length of the file.
				BUFFER_SIZE = input.Length < BUFFER_SIZE ? input.Length : BUFFER_SIZE;

				//Now read the next chunk into a byte buffer.
				long numBytesToRead = input.Length - contentOffset;
				long numBytesRead = 0;

				if (contentOffset > 0)
					Trace.TraceInformation("Resuming file at byte: {0}.", contentOffset.ToString());

				//Go to the part we want to start from.
				input.Seek(contentOffset, SeekOrigin.Begin);

				//Continue uploading chunks until we have none left.
				while (numBytesToRead > 0)
				{
					int CHUNK_SIZE = numBytesToRead > BUFFER_SIZE ? (int)BUFFER_SIZE : (int)numBytesToRead;
					byte[] bytes = new byte[CHUNK_SIZE];

					//Read may return anything from 0 to BUFFER_SIZE. 
					int n = input.Read(bytes, 0, CHUNK_SIZE);
					...
					req.ContentOffset = contentOffset + numBytesRead; //Set the offset so we can resume when we get an error.
					req.Timeout = -1;
					...
					using (Stream output = req.GetRequestStream())
					{
						try
						{
							//Upload the data to the stream.
							Trace.TraceInformation("Writing bytes.");
							output.Write(bytes, 0, bytes.Length);
						}
						finally
						{
							Trace.TraceInformation("Closing stream.");
							output.Close();
						}
					}
			...
			//Rename the file from the temporary name to the correct name - once we uploaded all the data successfully.
		...
		public void FtpDeleteDir(string dirPath, bool requireExists = false)
		{
			Trace.TraceInformation("FtpDeleteDir(dirPath: {0}, requireExists: {1})", dirPath, requireExists);
					...
					switch (res.StatusCode)
					{
						case FtpStatusCode.ActionNotTakenFileUnavailable:
							if (requireExists)
							{
								...
								Trace.TraceError(errmsg, dirPath, ex);
						...
						default:
							...
							Trace.TraceError(errmsg2, dirPath, ex);
		...
		public void FtpDeleteFile(string filePath, bool requireExists = false)
		{
			Trace.TraceInformation("FtpDeleteFile(filePath: {0}, requireExists: {1})", filePath, requireExists);
					...
					switch (res.StatusCode)
					{
						...
						case FtpStatusCode.ActionNotTakenFileUnavailableOrBusy:
							if (requireExists)
							{
								...
								Trace.TraceError(errmsg, filePath, ex);
						...
						default:
							...
							Trace.TraceError(errmsg2, filePath, ex);
		...
		public byte[] FtpDownload(string remoteFile, bool requireExists)
		{
			Trace.TraceInformation("FtpDownload(remoteFile: {0}, requireExists: {1})", remoteFile, requireExists);
				...
				try
				{
					switch (res.StatusCode)
					{
						...
						default:
							...
							Trace.TraceError(errmsg, fullRemotePath, ex);
		...
		/// <param name="requireExists">If false, and the remote file does not exist, a null will be returned instead.</param>
		/// <returns>The contents of the <paramref name="remoteFile"/>.</returns>
		public string FtpDownloadAsString(string remoteFile, bool requireExists)
		{
			Trace.TraceInformation("FtpDownloadAsString(remoteFile: {0}, requireExists: {1})", remoteFile, requireExists);
		...
		/// <summary>
		/// Returns the file size of a remote file.
		/// </summary>
		/// <param name="filePath">The path of the remote file. </param>
		/// <returns>The size of the file.</returns>
		public long FtpFileSize(string filePath)
		{
			Trace.TraceInformation("FtpFileSize(filePath: {0})", filePath);
			...
			{
				var res = (FtpWebResponse)req.GetResponse();
				long contentLength = res.ContentLength; //ContenLength will contain the File size.
			...
			catch (WebException ex)
			{
				...
				try
				{
					switch (res.StatusCode)
					{
						case FtpStatusCode.ActionNotTakenFileUnavailable:
						case FtpStatusCode.ActionNotTakenFileUnavailableOrBusy:
							Trace.TraceInformation("File unavailable or busy : {0}, {1}", res.StatusDescription, ex);
		...
		{
			Trace.TraceInformation("FtpFileExists(filePath: {0})", filePath);
		...
		public IEnumerable<string> FtpNListDir(string remotePath, bool requireExists = false)
		{
			Trace.TraceInformation("FtpNListDir(remotePath: {0}, requireExists: {1})", remotePath, requireExists);
			...
			try
			{
				using (var res = req.GetResponse())
				{
					if (res == null)
					{
						...
						Trace.TraceWarning(errmsgResNull, remotePath);
					...
					using (var resStream = res.GetResponseStream())
					{
						if (resStream == null)
						{
							...
							Trace.TraceWarning(errmsgResStreamNull, remotePath);
						...
						using (var sr = new StreamReader(resStream))
						{
							...
							Trace.TraceInformation("Starting read of WebResponseStream `{0}` for remote path `{1}` using StreamReader `{2}`.", resStream, remotePath, sr);

							while (!sr.EndOfStream)
							{
								var s = sr.ReadLine();

								Trace.TraceInformation("Read WebResponseStream line for remote path `{0}`: {1}", remotePath, s ?? "[NULL]");
					...
					switch (res.StatusCode)
					{
						case FtpStatusCode.ActionNotTakenFileUnavailable:
							if (requireExists)
							{
								...
								Trace.TraceError(errmsg, remotePath, ex);
						...
						default:
							...
							Trace.TraceError(errmsg2, remotePath, ex);
		...
		public void FtpRenameFile(string filePath, string newName)
		{
			Trace.TraceInformation("FtpRenameFile(filePath: {0}, newName: {1})", filePath, newName);
		...
		protected virtual void HandleDispose(bool disposeManaged)
		{
			...
			lock (_threadsSync)
			{
				foreach (var t in _threads)
				{
					...
					try
					{
						Trace.TraceInformation("Disposing `{0}`'s worker thread `{1}`.", this, t);
						t.Abort();
					}
					catch (Exception ex)
					{
						const string errmsg = "Disposing `{0}`'s worker thread `{1}` failed. Exception: {2}";
						Trace.TraceWarning(errmsg, this, t, ex);
		...
		static string SanitizeFtpTargetPath(string p)
		{
			...
			Trace.TraceInformation("Path `{0}` sanitized to `{1}`.", p, ret);
		...
		void WorkerThreadLoop()
		{
			while (!IsDisposed)
			{
				...
				Trace.TraceInformation("Processing job: {0}", job);
				...
				{
					Trace.TraceInformation("Processing job `{0}` failed. Exception: {1}", job, ex);
				...
				Trace.TraceInformation("Job `{0}` successfully processed.", job);
		...
		void WriteTestConnectionMessage(object userState, string msg)
		{
			...
			Trace.TraceInformation("TestConnectionMessage (userState: {0}): {1}", userState != null ? userState.ToString() : "[NULL]", msg);
		}
		...
		public bool CancelAsyncDownload(string localPath)
		{
			...
			Trace.TraceInformation("Canceling async download for `{0}`.", localPath);
		...
		public bool CancelAsyncUpload(string remotePath)
		{
			...
			Trace.TraceInformation("Canceling async upload for `{0}`.", remotePath);
		...
		public void DeleteDirectory(string targetPath, bool requireExists)
		{
			...
			Trace.TraceInformation("Deleting directory `{0}`.", targetPath);
		...
		public bool DeleteDirectoryAsync(string targetPath)
		{
			...
			Trace.TraceInformation("Asynchronously deleting directory `{0}`.", targetPath);
		...
		public byte[] Download(string remoteFile, bool requireExists = false)
		{
			...
			Trace.TraceInformation("Downloading file `{0}`.", remoteFile);
		...
		public string DownloadAsString(string remoteFile, bool requireExists = false)
		{
			...
			Trace.TraceInformation("Downloading file `{0}` as string.", remoteFile);
		...
		public bool DownloadAsync(string remotePath, string sourcePath)
		{
			...
			Trace.TraceInformation("Asynchronously Downloading file `{0}`.", remotePath);
		...
		public bool UploadAsync(string sourcePath, string remotePath)
		{
			...
			Trace.TraceInformation("Asynchronously uploading file `{0}` to `{1}`.", sourcePath, remotePath);
